...
def get_response(
    ...
    if api_key is None:
        try:
            ...
            print("You did not provide an API key, " 
...
def get_data(schema="data/local_hourly", devices=None, start_time=None, end_time=None):
    ...
    if not start_time:
        ...
        start_time = (dt.datetime.strptime(start_time, "%Y-%m-%d")
                      if isinstance(start_time, str) else
                      start_time)
        end_time = start_time if end_time is None else end_time
    
    
    if devices is None: devices = get_devices()
    retvals = []
...
def gather_data(output_folder, start_time=None):
    ...
    for schema in ALL_FIELDS:
        ...
        if df is not None:
            loc = output_folder/f"{start_time.strftime('%Y-%m-%d')}_{schema}.csv"
            df.to_csv(loc)
